...
export class SuperadminServices {
  private apiUrl = 'https://localhost:7277/api/SuperAdmin';
...
addUser(model: RegisterModel, role: string): Observable<any> {
  ...
  getAllTourCompanies(): Observable<any> {
    return this.http.get('https://localhost:7277/api/TourCompany').pipe(
      catchError(this.handleError)
    );
  }
  ...
  getAllHotelCompanies(): Observable<any> {
    return this.http.get('https://localhost:7277/api/HotelCompany').pipe(
      catchError(this.handleError)
    );
  }
  // ==================== UTILITY METHODS ====================
  ...
  getCompaniesByType(companyType: string): Observable<any> {
    const endpoints = {
      'hotel': 'https://localhost:7277/api/HotelCompany',
      'flight': 'https://localhost:7277/FlightCompany', 
      'carrental': 'https://localhost:7277/api/CarRental',
      'tour': 'https://localhost:7277/api/TourCompany'
    };
...
/// --------- Flight Mangment
  createFlightCompany(model: Iflightcompany): Observable<any> {
    ...
    return this.http.get<any[]>('https://localhost:7277/FlightCompany').pipe(
      catchError(this.handleError)
    );
  }
...
getCarRentalCompany(id: number): Observable<any> {
  ...
  return this.http.get(`https://localhost:7277/api/CarRental/${id}`).pipe(
...
getAllCarRentalCompanies(): Observable<any> {
  return this.http.get('https://localhost:7277/api/CarRental?pageSize=100').pipe(
 ...
 // ==================== REVIEW MANAGEMENT - UPDATED FOR BACKEND ====================
  ...
  getAllReviewsForAdmin(page: number = 1, pageSize: number = 20): Observable<any> {
    ...
    const reviewRequests = companyTypes.map(type => 
      this.http.get(`https://localhost:7277/api/Review/company?companyType=${type}&page=1&pageSize=100`).pipe(
        catchError(() => [])
      )
    );
  ...
  getReviewsByCompanyType(companyType: string, page: number = 1, pageSize: number = 20): Observable<any> {
    ...
    return this.http.get(`https://localhost:7277/api/Review/company?companyType=${companyType}&page=${page}&pageSize=${pageSize}`).pipe(
  ...
  getReviewStatistics(): Observable<any> {
    ...
    const statsRequests = companyTypes.map(type => 
      this.http.get(`https://localhost:7277/api/Review/stats?companyType=${type}`).pipe(
  ...
  deleteReviewAsAdmin(reviewId: number): Observable<any> {
    ...
    return this.http.delete(`https://localhost:7277/api/Review/${reviewId}`, {
      headers: this.getAuthHeaders()
    }).pipe(
  ...
  getReviewMetrics(): Observable<any> {
    ...
    return new Observable(observer => {
      ...
      const requests = companyTypes.flatMap(type => [
        this.http.get(`https://localhost:7277/api/Review/count?companyType=${type}`).pipe(
          catchError(() => of(0))
        ),
        this.http.get(`https://localhost:7277/api/Review/average?companyType=${type}`).pipe(
          catchError(() => of(0))
        )
      ]);
  ...
  getRecentReviewActivity(limit: number = 10): Observable<any[]> {
    ...
    const requests = companyTypes.map(type => 
      this.http.get(`https://localhost:7277/api/Review/company?companyType=${type}&page=1&pageSize=${limit}&sortBy=newest`).pipe(
        catchError(() => of([]))
      )
    );
