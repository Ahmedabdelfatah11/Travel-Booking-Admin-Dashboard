...
export class TourService {
  ...
  /**
   * ✅ GET: Dashboard stats for a specific tour company
   * @param companyId - The ID of the tour company
   */
  getDashboardStats(companyId: number): Observable<ITourStats> {
    const url = `${this.tourCompanyApiUrl}/${companyId}/stats`;
    return this.http.get<ITourStats>(url, { headers: this.getAuthHeaders() })
      .pipe(
        retry(2),
        catchError(this.handleError)
      );
