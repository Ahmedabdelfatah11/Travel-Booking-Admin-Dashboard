...
@Component({
  selector: 'app-tour-agency-booking-pending',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './tour-agency-booking-pending.html',
  styleUrls: ['./tour-agency-booking-pending.css']
})
export class TourAgencyBookingPending implements OnInit {
  bookings: BookingTourDto[] = [];
  pendingBookings: BookingTourDto[] = [];
  loading = false;
  ...
  ngOnInit(): void {
    this.loadTourCompanyIdAndPendingBookings();
  }

  private loadTourCompanyIdAndPendingBookings(): void {
  ...
  public loadPendingBookings(): void {
    ...
    this.bookingService.getBookingsByCompany(this.tourCompanyId).subscribe({
      next: (bookings) => {
        ...
        this.pendingBookings = this.bookings.filter(b => this.getDisplayStatus(b) === 'Pending');
        this.applySorting();
      },
      error: (err) => {
  ...
  calculatePagination(): void {
    this.totalPages = Math.ceil(this.pendingBookings.length / this.itemsPerPage);
  ...
  get paginatedBookings(): BookingTourDto[] {
    ...
    return this.pendingBookings.slice(start, start + this.itemsPerPage);
  }
